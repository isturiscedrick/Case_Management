...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Mobile top bar — hamburger trigger, only shown below md */}
      <div className="fixed inset-x-0 top-0 z-40 flex items-center gap-3 border-b border-slate-200 bg-[#12331F] px-4 py-3 md:hidden">
        <button
          type="button"
        ...
        <span className="font-serif text-sm font-semibold text-white">CMI Case Management</span>
      </div>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex h-screen w-60 flex-col border-r border-white/10 bg-[#12331F] shadow-sm transition-transform duration-200 md:transition-all
      ...
      >
        ...
        {/* Collapse toggle — desktop only; the mobile drawer closes via the backdrop/nav clicks instead */}
        <button
          onClick={() => setCollapsed((c) => !c)}
          aria-label={collapsed ? "Expand sidebar" : "Collapse sidebar"}
          className="absolute -right-3 top-7 z-10 hidden h-6 w-6 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-400 shadow-sm transition hover:border-[#B08D57] hover:text-[#B08D57] md:flex"
        >
          <ChevronsLeft
