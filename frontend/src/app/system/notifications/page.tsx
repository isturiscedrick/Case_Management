    ...
    <div className="min-h-full bg-[#F5F1E3] p-4 sm:p-6">
      <div className="mx-auto max-w-3xl">
        ...
        <section className="mt-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          {loading ? <p className="text-sm text-slate-400">Loading notifications...</p> : items.length === 0 ? <p className="text-sm text-slate-400">No notifications.</p> : <div className="space-y-2">{items.map((item) => <div key={item.notification_id} className="flex flex-col gap-3 rounded-lg bg-slate-50 p-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm text-slate-600">{item.message}</p><p className="mt-1 text-xs font-medium text-amber-600">{item.status === "pending" ? "Pending" : item.status}</p></div>{isAdmin && item.status === "pending" && <div className="flex gap-2"><button type="button" onClick={() => decide(item.notification_id, "decline")} className="rounded-lg border border-rose-200 px-3 py-2 text-xs font-medium text-rose-600 hover:bg-rose-50">Decline</button><button type="button" onClick={() => decide(item.notification_id, "approve")} className="rounded-lg bg-[#12331F] px-3 py-2 text-xs font-medium text-white hover:bg-[#1B4A2C]">Approve</button></div>}</div>)}</div>}
        </section>
      </div>
    </div>
  );
