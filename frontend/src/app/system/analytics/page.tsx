...
type StatusBucket = "Pending" | "Settled" | "Not Settled" | "Closed" | "Execution";

// Generic buckets shown on LA/NLRC/CA/SC cards.
const BUCKETS: StatusBucket[] = ["Pending", "Settled", "Not Settled", "Closed"];
// SEnA's card additionally shows "Execution" — case status can be
// "Execution" while a case hasn't progressed past SEnA yet.
const SENA_BUCKETS: StatusBucket[] = [...BUCKETS, "Execution"];

const OTHER_STAGE_KEYS: StageKey[] = ["la", "nlrc", "ca", "sc"];
...
const BUCKET_STYLES: Record<StatusBucket, { dot: string; bar: string; text: string }> = {
  ...
  Execution: { dot: "bg-indigo-500", bar: "bg-indigo-500", text: "text-indigo-700" },
};

// Category of the Total Judgment Award (totalPaid.category).
...
// Cycled through for company breakdown, since companies are dynamic data
// rather than a fixed set of keys like stages or categories.
const COMPANY_PALETTE: { dot: string; hex: string }[] = [
  { dot: "bg-sky-500", hex: "#0ea5e9" },
  { dot: "bg-violet-500", hex: "#8b5cf6" },
  { dot: "bg-fuchsia-500", hex: "#d946ef" },
  { dot: "bg-rose-500", hex: "#f43f5e" },
  { dot: "bg-amber-500", hex: "#f59e0b" },
  { dot: "bg-emerald-500", hex: "#10b981" },
  { dot: "bg-teal-500", hex: "#14b8a6" },
  { dot: "bg-slate-500", hex: "#64748b" },
];
...
// data, including the judgment award specification fields, not just the
...
function classifyStatus(item: CaseItem, stage: StageKey): StatusBucket {
  ...
  // "Execution" only ever applies to the SEnA card — it's a case-level
  // status, but we intentionally don't surface it on LA/NLRC/CA/SC.
  if (stage === "sena" && item.status === "Execution") return "Execution";
...
function StageStatusCard({
  ...
  return (
    <div className={`rounded-xl border ${meta.ring} bg-white p-4 shadow-sm`}>
...
export default function AnalyticsPage() {
  ...
  const stageBreakdown = useMemo(() => {
    const result: Record<StageKey, Record<StatusBucket, number>> = {
      sena: { Pending: 0, Settled: 0, "Not Settled": 0, Closed: 0, Execution: 0 },
      la: { Pending: 0, Settled: 0, "Not Settled": 0, Closed: 0, Execution: 0 },
      nlrc: { Pending: 0, Settled: 0, "Not Settled": 0, Closed: 0, Execution: 0 },
      ca: { Pending: 0, Settled: 0, "Not Settled": 0, Closed: 0, Execution: 0 },
      sc: { Pending: 0, Settled: 0, "Not Settled": 0, Closed: 0, Execution: 0 },
    };
  ...
  // Case count per company, using the same date-range-filtered `cases` as
  // every other breakdown on this page.
  const companyBreakdown = useMemo(() => {
    ...
    return Array.from(map.entries())
      .map(([name, count], i) => ({ name, count, color: COMPANY_PALETTE[i % COMPANY_PALETTE.length] }))
      .sort((a, b) => b.count - a.count);
  }, [cases]);

  const companyCaseTotal = useMemo(
    () => companyBreakdown.reduce((sum, c) => sum + c.count, 0),
    [companyBreakdown]
  );

  // Conic-gradient pie (by case count) for the company split.
  const companyPieGradient = useMemo(() => {
    if (companyCaseTotal === 0) return "conic-gradient(#e2e8f0 0deg 360deg)";
    let cursor = 0;
    const stops = companyBreakdown.map((c) => {
      const start = cursor;
      const sliceDeg = (c.count / companyCaseTotal) * 360;
      cursor += sliceDeg;
      return `${c.color.hex} ${start}deg ${cursor}deg`;
    });
    return `conic-gradient(${stops.join(", ")})`;
  }, [companyBreakdown, companyCaseTotal]);
    ...
    <div className="h-full min-w-0 space-y-4 overflow-y-auto bg-[#F5F1E3] p-4">
      ...
      <div>
        ...
        <div className="grid gap-3 lg:grid-cols-2 xl:grid-cols-3">
          <StageStatusCard stage="sena" counts={stageBreakdown.sena} buckets={SENA_BUCKETS} />
          {OTHER_STAGE_KEYS.map((stage) => (
            <StageStatusCard key={stage} stage={stage} counts={stageBreakdown[stage]} buckets={BUCKETS} />
          ))}
        </div>
      </div>
        ...
        <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto]">
            ...
            {companyBreakdown.length === 0 ? (
              ...
              <div className="space-y-3">
                {companyBreakdown.map(({ name, count, color }) => {
                  const pct = companyCaseTotal > 0 ? (count / companyCaseTotal) * 100 : 0;
                  return (
                    <div key={name} className="flex items-center gap-3">
                      ...
                      <div className="relative h-6 flex-1 overflow-hidden rounded-md bg-slate-100">
                        <div
                          className={`h-full rounded-md ${color.dot} opacity-80`}
          ...
          <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="relative h-36 w-36">
              ...
              <div className="absolute inset-3 flex flex-col items-center justify-center rounded-full bg-white">
                <span className="text-lg font-semibold text-[#12331F]">{companyCaseTotal}</span>
                <span className="text-[10px] text-slate-400">cases</span>
              </div>
            </div>
            <div className="w-full space-y-1">
              {companyBreakdown.map(({ name, count, color }) => {
                const pct = companyCaseTotal > 0 ? Math.round((count / companyCaseTotal) * 100) : 0;
                return (
                  <div key={name} className="flex items-center justify-between gap-2 text-[11px]">
                    <span className="flex items-center gap-1.5 truncate text-slate-500">
