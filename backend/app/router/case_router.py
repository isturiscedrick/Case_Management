...
def create_case(payload: CaseCreate, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    return case_service.create_case(db, payload, current_user)


@router.put("/{case_id}", response_model=CaseOut)
def update_case(case_id: int, payload: CaseUpdate, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    return case_service.update_case(db, case_id, payload, current_user)


@router.post("/{case_id}/toggle-archive", response_model=CaseOut)
def toggle_archive(case_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    return case_service.toggle_archive(db, case_id, current_user)


@router.post("/{case_id}/close", response_model=CaseOut)
def close_case(case_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    return case_service.close_case(db, case_id, current_user)


@router.post("/{case_id}/unclose", response_model=CaseOut)
def unclose_case(case_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
