...
def get_history(
    ...
    return history_crud.list_history(db, search=search, action=action, page=page, page_size=page_size, performed_by_user_id=performed_by_user_id)


def get_case_history(db: Session, case_id: int) -> List[CaseHistory]:
    return history_crud.list_case_history(db, case_id)